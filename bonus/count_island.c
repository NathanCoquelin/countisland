...
{
    if (world[y + 1] == NULL) {
        my_mark_as_seen(world, 0, x, count);
        return;
    }
    if (world[y + 1][x] == 'X')
        my_mark_as_seen(world, y + 1, x, count);
    if (y == 0) {
        my_mark_as_seen(world, get_max_vertical(world), x, count);
        return;
    }
    if (world[y - 1][x] == 'X')
        my_mark_as_seen(world, y - 1, x, count);
...
{
    if (world[y][x + 1] == '\0') {
        my_mark_as_seen(world, y, 1, count);
        return;
    }
    if (world[y][x + 1] == 'X')
        my_mark_as_seen(world, y, x + 1, count);
    if (x == 0) {
        my_mark_as_seen(world, y, get_max_horizontal(world), count);
        return;
    }
    if (world[y][x - 1] == 'X')
        my_mark_as_seen(world, y, x - 1, count);
